...
let rec print_trace = function
    [] -> print_newline()
  | [x] -> print_endline (string_of_termbruijn x)
  | x::l -> print_endline (string_of_termbruijn x); print_string " -> " ; print_trace l
;;
...
(* wrong usage *)      
| _ -> failwith "Usage: dune exec untypedbruijn [file]"
