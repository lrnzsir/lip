...
let rec is_nat s z = function
  | Var x when x = z -> true
  | App(Var x, t) when x = s -> is_nat s z t
  | _ -> false

let string_of_nat t = 
  let rec string_of_nat' = function
  | Var _ -> 0
  | App(_, t') -> 1 + string_of_nat' t'
  | _ -> failwith "Not a Nat of Church"
in string_of_int (string_of_nat' t)
...
let rec string_of_term = function
  ...
  | t when equiv t t_fls -> "add"
  | Abs(s, Abs(z, t)) when is_nat s z t -> string_of_nat t
